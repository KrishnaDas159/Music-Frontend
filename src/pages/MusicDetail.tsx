...
const MusicDetail = () => {
  ...
  useEffect(() => {
    const fetchSong = async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch(`https://music-backend-final.onrender.com/api/songs/${id}`);
        if (!res.ok) throw new Error("Song not found");
        const data = await res.json();
        setSongData(data);

        // Fetch current curve price
        if (data.curveId) {
          const priceRes = await fetch(
              `https://music-backend-final.onrender.com/api/curve/price/${data.curveId}/${quantity}`
          );
          const priceData = await priceRes.json();
          setCurvePrice(priceData.price || data.stats.tokenPrice);
        }
      } catch (err) {
    ...
    setIsWalletConnected(!!localStorage.getItem("walletConnected"));
    fetchSong();
  }, [id, quantity]);
  ...
  const handleConfirmPurchase = async () => {
    try {
      const response = await fetch("https://music-backend-final.onrender.com/api/buy-tokens", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          creatorAddress: songData.creatorAddress,
        }),
      });
      if (!response.ok) throw new Error("Failed to purchase tokens");
      const result = await response.json();
      alert(`Successfully purchased ${quantity} tokens! Transaction: ${result.transactionId}`);
  ...
  const totalPrice = (parseFloat(curvePrice) * quantity).toFixed(2);
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {songData?.stats && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    { label: "Token Price", value: curvePrice },
                ...
                <div className="text-center space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold text-yellow-600">{curvePrice} SUI</span>
                    </div>
                    <div className="flex justify-between items-center">
                    ...
                    <div className="flex justify-between items-center text-lg font-semibold">
                      <span>Total:</span>
                      <span className="text-yellow-600">{totalPrice} SUI</span>
                    </div>
                  </div>
